...
TRANSACTIONS = {
    "checking": [
        {"date": "2026-06-27", "description": "Salary — Rossi & Partners", "amount": 2450.00},
        {"date": "2026-06-28", "description": "Supermarket Esse-Piu", "amount": -74.30},
        {"date": "2026-06-30", "description": "Card payment — Trattoria da Peppe", "amount": -38.50},
        {"date": "2026-07-01", "description": "Utility bill — electricity", "amount": -61.20},
        {"date": "2026-07-03", "description": "ATM withdrawal", "amount": -100.00},
    ],
    "savings": [
        {"date": "2026-06-15", "description": "Transfer from checking", "amount": 200.00},
        {"date": "2026-06-30", "description": "Interest credit", "amount": 3.40},
    ],
}


@app.get(
    "/v1/accounts/{account_type}/transactions",
    dependencies=[Depends(verify_token)],
)
def list_transactions(account_type: str):
    if account_type not in TRANSACTIONS:
        raise HTTPException(
            status_code=422,
            detail=f"Unknown account type: {account_type}",
        )
    # The five most recent movements, newest last.
    return {
        "account_type": account_type,
        "transactions": TRANSACTIONS[account_type][-5:],
    }
...
def check_card_type(card_type: str) -> None:
    if card_type not in CARD_TYPES:
        raise HTTPException(
            status_code=422,
            detail=f"Unknown card type: {card_type}",
        )


@app.post("/v1/cards/block", dependencies=[Depends(verify_token)])
def block_card(request: CardRequest):
    check_card_type(request.card_type)
    return {"card_type": request.card_type, "status": "blocked"}


@app.post("/v1/cards/replace", dependencies=[Depends(verify_token)])
def replace_card(request: CardRequest):
    check_card_type(request.card_type)
    return {
    ...
    TRANSACTIONS["checking"].append(
        {
            ...
            "description": f"Transfer to {request.recipient_name}",
